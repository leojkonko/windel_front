...
class ScreensService
{
    ...
    private function loadFolder(string $folder)
    {
        ...
        foreach ($php_files as $php_file) {
            ...
            if ($is_modules) {
                $class_name = "\\Ellite\\" . substr($file, strpos($file, "modules") + 8, -4);
            }
            else {
    ...
    public function metricsLayout()
    {
        ...
        $return['Contatos no último mês'] = $layout['Contatos no último mês'];
        $return['Contatos de produtos no último mês'] = $layout['Contatos de produtos no último mês'];
        $return['Interessses de treinamentos no último mês'] = $layout['Interessses de treinamentos no último mês'];
        $return['Total de produtos'] = $layout['Total de produtos'];
        $return['Total de artigos'] = $layout['Total de artigos'];
